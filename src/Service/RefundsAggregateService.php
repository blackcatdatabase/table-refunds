 ...
 */
final class RefundsAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private RefundRepository $refundRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'refunds' => $this->refundRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\Refunds\ModuleException("Record {$idStr} not found for locking.");
            }
