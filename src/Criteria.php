 ...
 * - whitelist filtrů: [ 'id', 'payment_id', 'amount', 'currency', 'reason', 'status', 'created_at', 'details' ]
 * - whitelist pro LIKE hledání: [ 'currency', 'reason', 'status' ]
 */
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        // filtry (rovnosti / IN)
        $allowed = array_fill_keys([ 'id', 'payment_id', 'amount', 'currency', 'reason', 'status', 'created_at', 'details' ], true);
        ...
        if ($this->search !== null) {
            $searchCols = [ 'currency', 'reason', 'status' ];
            $likeParts = [];
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'id', 'payment_id', 'amount', 'currency', 'reason', 'status', 'created_at', 'details' ], true);
            $parts = [];
